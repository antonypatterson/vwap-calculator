...
public class VWAPData {
    ...
    private LocalDateTime lastRestoredDataTimestamp = now.minusMinutes(oldestTradeMinutes + 1);
    ...
    public synchronized void addDataPoint(Trade trade) {
        ...
        while (!tradeQueueNewest.isEmpty() && tradeQueueNewest.peek().getTimestamp().isBefore(currentTime.minusHours(1))) {
        ...
        while (!tradeQueueOld.isEmpty() && tradeQueueOld.peek().getTimestamp().isBefore(currentTime.minusHours(1))) {
    ...
    public Queue<Trade> getNewestQueue() {
        return tradeQueueNewest;
    }

    public Queue<Trade> getOldQueue() {
        return tradeQueueOld;
    }
